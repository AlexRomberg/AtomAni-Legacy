...
Matrix._inv4 = function(c, a) {
    ...
    if (Math.abs(det) > 1.0e-6) {
        ...
        var invdet = 1.0 / det;
        c[0] = [
            m11 * invdet, -m21 * invdet,
            m31 * invdet, -m41 * invdet
        ];
        c[1] = [-m12 * invdet,
            m22 * invdet, -m32 * invdet,
            m42 * invdet,
        ];
        c[2] = [
            m13 * invdet, -m23 * invdet,
            m33 * invdet, -m43 * invdet,
        ];
        c[3] = [-m14 * invdet,
            m24 * invdet, -m34 * invdet,
            m44 * invdet
        ];
